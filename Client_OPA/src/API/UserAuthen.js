...
class UserAuthenAPIClass {
  uploadProfilePic = (files, user) =>
    new Promise((resolve, reject) => {
      ...
      Axios.post('http://localhost/uh/addProfilePic', formData).then((res) => {
        if (res.data.success) {
          resolve(true);
        } else {
          reject(false);
  ...
  login = ({ username, password, remMe }) =>
    new Promise((resolve, reject) => {
      Axios.get('http://localhost/uh/login', {
        params: {
          username,
          password,
          remMe,
  ...
  authen = () =>
    new Promise((resolve, reject) => {
      Axios.get('http://localhost/uh/authen', { withCredentials: true })
        .then((res) => {
    ...
    new Promise((resolve, reject) => {
      ...
      if (username !== '' || password !== '' || email !== '') {
        if (password.length < 8) {
          ...
          Axios.post('http://localhost/uh/signup', sendData).then((res) => {
  ...
  logout = () =>
    new Promise((resolve) => {
      Axios.get('http://localhost/uh/logout', { withCredentials: true }).then(() => {
        resolve('hi');
      });
    });
    ...
    new Promise((resolve, reject) => {
      ...
      if (username !== '' || password !== '' || email !== '') {
        if (password.length < 8) {
          ...
          Axios.post('http://localhost/uh/verify', sendData).then((res) => {
